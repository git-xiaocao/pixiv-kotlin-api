...
import me.xiaocao.dto.*
import me.xiaocao.entity.Comment
import okhttp3.Interceptor
import okhttp3.Response

@SuppressWarnings("unused")
class ApiClient(private var userAccount: UserAccount, private val updateUserAccount: (data: UserAccount) -> Unit) : Interceptor {
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        val response = chain.proceed(
            request.newBuilder().apply {
                header("Authorization", userAccount.accessToken)
            }.build()
        )
        ...
        if (HttpStatusCode.BadRequest.value == response.code) {
            if (true == response.body?.byteString()?.utf8()?.contains("OAuth")) {
                return chain.proceed(
                    request.newBuilder().apply {
                        runBlocking {
                            userAccount = authClient.refreshAuthToken(userAccount.refreshToken).also {
    ...
    suspend inline fun <reified T : IPixivListData> next(url: String): T {
        return nextResponse(url).body()
    }

    suspend fun nextResponse(url: String): HttpResponse {
        return httpClient.get(url.replaceFirst(TARGET_HOST, TARGET_IP))
    }
     ...
     *  @param restrict
     * */
    suspend fun getUserIllustBookmarks(userId: Int, restrict: Restrict): Illusts {
        return httpClient.get {
     ...
     *  @param restrict
     * */
    suspend fun getUserNovelBookmarks(userId: Int, restrict: Restrict): Illusts {
        return httpClient.get {
     ...
     *  @param type 类型
     * */
    suspend fun getUserIllusts(userId: Int, type: IllustType): Illusts {
        return httpClient.get {
     ...
     *  @param userId 用户ID
     * */
    suspend fun getUserNovels(userId: Int): Novels {
        return httpClient.get {
     ...
     *  @param type 类型
     * */
    suspend fun getRecommendedIllusts(type: IllustType): Illusts {
        return httpClient.get {
     ...
     * 获取推荐小说
     * */
    suspend fun getRecommendedNovels(): Novels {
        return httpClient.get {
     ...
     *  @param restrict
     * */
    suspend fun getFollowingUsers(userId: Int, restrict: Restrict): Users {
        return httpClient.get {
     ...
     * */
    suspend fun getFollowerNewIllusts(restrict: Restrict? = null): Illusts {
        return httpClient.get {
     ...
     *  @param restrict 为null获取全部(all)
     * */
    suspend fun getFollowerNewNovels(restrict: Restrict? = null): Illusts {
        return httpClient.get {
     ...
     * @param illustId 插画ID
     * */
    suspend fun getUgoiraMetadata(illustId: Int): UgoiraMetadata {
        return httpClient.get {
     ...
     * 搜索用户
     * */
    suspend fun searchUsers(word: String): Users {
        return httpClient.get {
     ...
     * @param isNovel 是否为小说
     * */
    suspend fun getBookmarkTags(userId: Int, restrict: Restrict, isNovel: Boolean): Users {
        return httpClient.get {
     ...
     * @param tags 收藏标签
     * */
    suspend fun addBookmark(illustId: Int, restrict: Restrict, tags: List<String> = emptyList(), isNovel: Boolean) {
        return httpClient.post {
     ...
     * @param isNovel 是否为小说
     * */
    suspend fun deleteBookmark(illustId: Int, isNovel: Boolean) {
        return httpClient.post {
     ...
     * @param restrict
     * */
    suspend fun addFollow(userId: Int, restrict: Restrict) {
        return httpClient.post {
     ...
     * */
    suspend fun addCommand(illustId: Int, comment: String = "", stampId: Int?, parentCommentId: Int? = null): Comment {
        return httpClient.post {
     ...
     * @param commentId 评论ID
     * */
    suspend fun deleteCommand(commentId: Int) {
        return httpClient.post {
